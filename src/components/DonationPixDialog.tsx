...
export default function DonationPixDialog({ open, onOpenChange }: DonationPixDialogProps) {
  ...
  const generate = async (value: number) => {
    ...
    try {
      const code = Pix.generate({
        ...
        city: "CANELINHA",
        amount: value,
        txid: `DLPR${Date.now().toString(36).slice(-8)}`, // txid único (até 25 caracteres)
      });
      setBrCode(code);
  ...
  const isPresetAmount = PRESETS.includes(numericAmount as (typeof PRESETS)[number]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader className="text-center space-y-2">
          ...
          <DialogTitle className="text-2xl font-bold text-davi-green-dark">Doe via PIX</DialogTitle>
        ...
        <div className="space-y-2">
          ...
          {amount !== "" && numericAmount > 0 && !isPresetAmount && (
            <Button
            ...
            >
              Gerar PIX para R$ {numericAmount.toFixed(2)}
            </Button>
          )}
        </div>
        ...
        {brCode && (
          <div className="space-y-4 border-t pt-4">
            <div className="text-center">
              <p className="text-lg font-bold text-davi-green-dark mb-2">Valor: R$ {numericAmount.toFixed(2)}</p>
              {qr && (
            ...
            <div className="space-y-2">
              <label className="text-sm font-semibold text-davi-green-dark">Código PIX copia e cola:</label>
              <Textarea
                readOnly
                value={brCode}
                rows={4}
        ...
        <DialogFooter className="gap-2 pt-4">
          {brCode ? (
            <div className="flex gap-2 w-full">
              <Button onClick={copy} className="flex-1 bg-davi-blue hover:bg-davi-blue/90 text-white">
